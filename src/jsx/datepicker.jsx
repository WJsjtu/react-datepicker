			...
			var self = this, tbody = [];
		  	for(i = 0; i < 6; i++){
		  		...
		  		for(j = 0; j < 7; j++){
		  			...
		  			tr.push(<td key={name} className={className} onClick={onClick}>{self.state[itemPreffix + name]}</td>);
		  		}
		...
		render: function(){
			...
			for(; i < 12; i++){
				temp = getClassSet( self.state[classPreffix + i],
		  		...
		  		spans.push(<span key={"m-" + i} className={className} onClick={onClick}>{yearMonths[self.state[itemPreffix + i] - 1] + "月"}</span>);
			}
		...
		render: function(){
			...
			for(; i < 12; i++){
				temp = getClassSet( self.state[classPreffix + i],
		  		...
		  		className = temp[0], onClick = temp[1];
		  		spans.push(<span key={"y-" + i} className={className} onClick={onClick}>{self.state[itemPreffix + i]}</span>);
			}
			return <tbody><tr><td colSpan="7">{spans}</td></tr></tbody>;
		}
	}),
		...
		render: function() {
			var self = this; stateArgs = self.state, propArgs = self.props,
				...
				prevTh = <th className={classPrefix + "prev"}>
							<i className={classPrefix + "prev-icon"} onClick={self.goIcon.bind(self, 1)}></i>
						</th>,
				nextTh = <th className={classPrefix + "next"}>
							<i className={classPrefix + "next-icon"} onClick={self.goIcon.bind(self, -1)}></i>
						</th>;
			if(stateArgs.panel == 1){
				table = <table className={tableClass}>
							<thead>
		  					...
		  					<DatepickerDayTbody from={propArgs.from} 
												to={propArgs.to} 
												current={stateArgs.current} 
				...
				table = <table className={tableClass}>
							<thead>
								<tr className={headClass}>{prevTh}
									<th colSpan="5" className={switchClass}>
										<div className={selectClass} onClick={self.changePanel}>{stateArgs.current.y}</div>
									</th>{nextTh}
								</tr>
							</thead>
				...
				table = <table className={tableClass}>
							<thead>
								<tr className={headClass}>{prevTh}
									<th colSpan="5" className={switchClass}>
										<div className={selectClass}>{start}-{start + 11}</div>
									</th>{nextTh}
								</tr>
							</thead>
							<DatepickerYearTbody from={propArgs.from} 
												 to={propArgs.to} 
												 current={stateArgs.current} 
												 select={self.selectPanel.bind(self, 2)}/>
						</table>;
			}
		    return <div className="am-datepicker" style={{display: "block"}} onWheel={self.onWheel}>
